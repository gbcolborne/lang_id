...
"""
Pre-train BERT on masked language model and next sentence prediction tasks.
...
from pytorch_pretrained_bert.modeling import BertForPreTraining, BertConfig, WEIGHTS_NAME, CONFIG_NAME
...
class BERTDataset(Dataset):
    def __init__(self, corpus_path, tokenizer, seq_len, encoding="utf-8", corpus_lines=None, on_memory=True):
        ...
        if on_memory:
            ...
            if len(self.all_docs) == 0 or doc != self.all_docs[-1]:
                if len(doc):
                    self.all_docs.append(doc)
                    self.sample_to_doc.pop()
...
def main():
    ...
    # Prepare model
    if pretrained:
        model = BertForPreTraining.from_pretrained(args.bert_model_or_config_file)
    else:
